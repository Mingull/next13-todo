...
const usePagination = ({ total, siblings = 1, boundaries = 1, page, initialPage = 1, onChange }: PaginationParams) => {
	const _total = Math.max(Math.trunc(total), 0);
	const [activePage, setActivePage] = useUncontrolledState({
		value: page,
		onChange,
